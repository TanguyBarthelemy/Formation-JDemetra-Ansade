...
window(var_aux, start = 2020, end = c(2020, 12)) <- 1
plot(var_aux)

spec_td_t <- RJDemetra::x13_spec(
    spec_td,
    usrdef.varEnabled = TRUE,
...
refreshed_spec <- rjd3x13::x13_refresh(
    current_result_spec,
...
# take into consideration what will happen to point revised
# in the (recent) past, not only new points
